...
def rotate_and_crop(
    ...
    """
    Rotate `array` by `angle` (degrees CCW) and optionally crop the result.
    Uses precomputed remap tables for efficiency.
    ...
    if array.dtype == np.bool_:
        ...
        if use_cuda:
            try:
                ...
                # Perform remap on GPU
                gpu_result = cv2.cuda.remap(gpu_array, gpu_map_x, gpu_map_y, 
                                           cv2.INTER_CUBIC)
